<?php
	...
	if(isset($_GET["id_cuenta"])){
		...
		$sql .= " is_banned = '0'";
		$sql .= " WHERE id_cuenta = ".$_GET["id_cuenta"];
		...
		$_SESSION["mensaje-sistema"] = "<div class='alert alert-danger alert-dismissible' role='alert'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button>Problema con la asignaci&oacute;n de la cuenta favor intentar nuevamente.</div>";
	}
	
	header("location: ../pages/ver-corredores.php");//Entrada a la pagina sin el formulario correspondiente.
?>