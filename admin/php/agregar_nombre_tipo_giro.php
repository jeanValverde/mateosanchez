<?php
	...
	if(isset($_POST['nombre_tipo_giro']) && !empty($_POST['nombre_tipo_giro'])){
		$nombre_tipo_giro = ucwords($_POST['nombre_tipo_giro']);
	}else{
		$is_fail = 1;
	}
	...
	if($is_fail == 0 && $is_test == 0){
		...
		$sql = "INSERT INTO tipo_giros ";
		$sql .= "(nombre_tipo_giro) "; //:param_01
		$sql .= "VALUES ";
		$sql .= "(:param_01)";
		...
		$inserta -> bindValue(':param_01', $nombre_tipo_giro);
		
		//Se genera el ejecutar de la sentencia con una salvaguarda por si falla
		$inserta -> execute();
		$id_tipo_giro = $conexion -> lastInsertId();
		$conexion -> commit();
	}
	
	$sql = "SELECT * FROM tipo_giros";
	$cursor = $conexion -> query($sql);
	while($tipo_giro = $cursor->fetch()){
		?>
		<option value="<?php echo $tipo_giro["id_tipo_giro"]; ?>" <?php if($id_tipo_giro == $tipo_giro["id_tipo_giro"]){echo "selected";} ?>><?php echo utf8_encode($tipo_giro["nombre_tipo_giro"]); ?></option>
		<?php
	}
?>