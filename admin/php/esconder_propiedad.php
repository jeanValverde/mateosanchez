<?php
	...
	if(isset($_GET["id_propiedad"])){
		...
		if($is_test == 0){
			...
			$sql_codigo_grupo_propiedad = "DELETE FROM codigos_grupos_propiedades WHERE cod_propiedad='".$propiedad["cod_propiedad"]."'";
			$borra = $conexion->prepare($sql_codigo_grupo_propiedad);
			$borra->execute();
			
			//Codigo de envio de correo para cuenta de barbara u otro coordinador
			$subject = "Alerta por DADA DE BAJA PROPIEDAD a sistema mateosanchez.cl.";
			$message = "Este es un mensaje automatico avisando que la propiedad codigo: ".$cod_propiedad." fue dado de baja en el sistema de mateosanchez.cl \n\n";
