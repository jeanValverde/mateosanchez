...
<html lang="en">
    ...
    <body id="home" class="wide">
                    ...
                    <div class="container">
                        <div class="row">
							...
							<div class="col-md-12 content property-listing" id="content">
								<form action="reenviar_mensaje.php" method="post">
									<input type="hidden" name="id_mensaje" value="<?php echo $mensaje["id_mensaje"];?>"></input>
									<div class="form-group col-md-12">
										<label>Escoja nuevo agente destino (Esta opci&oacute;n es aparte al resto del formulario).</label>
									</div>
									<div class="form-group col-md-8">
										<select class="form-control" name="id_cuenta" id="id_cuenta">
											<?php
											$sql_cuenta = "SELECT * FROM cuentas WHERE is_banned = 0 and can_intranet = 1";
											$cursor_cuenta = $conexion -> query($sql_cuenta);
											while($cuenta = $cursor_cuenta -> fetch()){
											?>
											<option value="<?php echo $cuenta["id_cuenta"]; ?>" <?php if($_COOKIE["id_cuenta"] == $cuenta["id_cuenta"]){echo "SELECTED";}?>><?php echo $cuenta["nombre_persona"]; ?></option>
											<?php
											}
											?>
									...
									<div class="form-group col-md-2">
										<button type="submit" class="btn btn-success pull-left">Reenviar nota</button>
									</div>
								</form>
								...
								<form id="form-notepad-ficha" action="actualizar_mensaje.php" method="post">
									...
									<div class="col-md-12">
										<div class="row">
											...
											<div class="form-group col-md-4">
												...
												<select class="form-control" id="id_estado_mensaje" name="id_estado_mensaje" <?php if($mensaje["id_estado_mensaje"] == 1){echo "style='background-color: ##FE3200'";}elseif($mensaje["id_estado_mensaje"] == 2){echo "style='background-color: #FFC652'";}else{echo "style='background-color: #00A900'";}?>>
													<?php
													$sql_estado_mensaje = "SELECT * FROM estado_mensajes";
													$cursor_estado_mensaje = $conexion -> query($sql_estado_mensaje);
													while($estado_mensaje = $cursor_estado_mensaje -> fetch()){
														?>
														<option value="<?php echo $estado_mensaje["id_estado_mensaje"]; ?>" <?php if($mensaje["id_estado_mensaje"] == $estado_mensaje["id_estado_mensaje"]){echo "SELECTED";} ?>><?php echo $estado_mensaje["nombre_estado_mensaje"]; ?></option>
														<?php
													}
													?>
												</select>
									...
									<div class="form-group col-md-4">
										...
										<select class="form-control" id="id_region_nota" name="id_region">
											<option value="0">No considerado</option>
											<?php
									...
									<div class="form-group col-md-4">
										<label>Comuna</label>
										<select class="form-control" id="id_comuna_nota" name="id_comuna">
											<option value="0">No considerado</option>
											<?php
									...
									<div class="form-group col-md-4">
										<label>Sector</label>
										<select class="form-control" id="id_sector_nota" name="id_sector">
											<option value="0">No considerado</option>
											<?php
									...
									<div class="col-lg-2">
										<div class="form-group">
											<label>
												Publicidad en terreno <input type="checkbox" name="publicidad_terreno_mensaje" value="1" <?php if($mensaje["publicidad_terreno_mensaje"]){echo 'checked';}?>>
											</label>
										</div>
									</div>
									...
									<div class="col-lg-2">
											<div class="form-group">
												<label>
													Llamado Telef&oacute;nico <input type="checkbox" name="llamado_telefonico" value="1" <?php if($mensaje["llamado_telefonico"]){echo 'checked';}?>>
												</label>
											</div>
										</div>
			...
			<div class="modal-content">
			  ...
			  <div class="modal-body">
				...
				if(!empty($mensaje["codigo_propiedad_cliente"]) && is_numeric($mensaje["codigo_propiedad_cliente"])){
					...
					$sql_propiedad .= "WHERE propiedades.cod_propiedad=".$mensaje["codigo_propiedad_cliente"]." AND is_hidden=0";
		...
		<script>
			...
			$(document).ready(function(){
				$("#id_comuna_nota").change(function(){
					$.post("../admin/php/selector_sector_buscador.php",{ id:$(this).val() },function(data){$("#id_sector_nota").html(data);})
				});
			})
			
			$(document).ready(function(){
				$("#id_region_nota").change(function(){
					$.post("../admin/php/selector_comuna_buscador.php",{ id:$(this).val() },function(data){$("#id_comuna_nota").html(data);})
				});
			})
