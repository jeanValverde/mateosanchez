<?php
	...
	if(!isset($_POST['correo_cuenta']) || !isset($_POST['clave_cuenta'])){
		...
		$sql = "SELECT * FROM cuentas WHERE correo_cuenta='".$correo_cuenta."' AND clave_cuenta='".$clave_cuenta."' AND can_intranet = 1";
		...
		if($cuenta = $cursor -> fetch()){
			...
			$_SESSION["is_intranet_active"] = 1;
			setcookie("is_intranet_active", 1, time()+28800,'/'); //Cookie con duracion de 8 horas
		}else{
			$_SESSION["mensaje-login"] = "Informacion incorrecta o no tiene permiso para ingresar.";
			$is_fail = 1;
		}
	}
